...
static uint64_t t_tx_m2_actual = 0;
...
void run2(DWM_Module* module, volatile bool* isr_flag, volatile bool* button_flag)
{
    ...
    switch(remote_state)
    {
        ...
        case REMOTE_SEND_M2:
        {
            ...
            if(*button_flag){
                response_payload[0] = MSG_TYPE_STANDBY;
                *button_flag = false;
            ...
            if(*isr_flag)
            {
                *isr_flag = false;
                ...
                if(txfrs)
                {
                    ...
                    // // Save send m2 timestamp
                    // t_tx_m2_actual = read_timestamp(module, 0x17);
                    // Serial.println(t_tx_m2_actual);
...
/// NEW ADDITION FOR DS TWR *******
            ...
            if(*isr_flag)
            {
                *isr_flag =  false;
                ...
                if(rxdfr && rxfcg && ldedone){
                    ...
                    t_rtt_2 = ts_diff(t_rx_m3,t_tx_m2);
        ...
        case REMOTE_SEND_M4:
        {
            ...
            for(int i = 0; i < 5; i++){
                // NEW: SEND T_RTT_2 INSTEAD OF T_REPLY
                response_payload[1+i] = (t_rtt_2 >> (8*i)) & 0xFF;
